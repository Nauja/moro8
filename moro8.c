...
char* moro8_print(const struct moro8_vm* vm)
{
	...
	ptr = moro8_print_register_word(ptr, "C: ", vm->registers.p.c);
	*ptr++ = '\n';
		...
		if (state == MORO8_STATE_IDLE)
		{
			...
			for (size_t j = 0; i < size; ++j, ++i)
			{
				if (buf[j] == ':')
				{
					...
					{
						value_buffer = &vm->registers.p.n;
					}
					else if (buf[0] == 'V')
					{
						value_buffer = &vm->registers.p.v;
					}
					else if (buf[0] == 'C')
					{
						value_buffer = &vm->registers.p.c;
					}
					else if (buf[0] == 'Z')
					{
						value_buffer = &vm->registers.p.z;
					}
					else
					{
						// Unknown label
...
{ \
	...
	case MORO8_OP_ADDC_ZP_X:
		...
		break;
	case MORO8_OP_ADDC_ABS:
		MORO8_ADC(MORO8_MEM_ABS);
		break;
	case MORO8_OP_ADDC_ABS_X:
		MORO8_ADC(MORO8_MEM_ABS_X);
		break;
	case MORO8_OP_ADDC_ABS_Y:
		MORO8_ADC(MORO8_MEM_ABS_Y);
		break;
	case MORO8_OP_ADDC_IND_X:
