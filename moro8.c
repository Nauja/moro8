...
#define MORO8_PC vm->state.registers.pc
#define MORO8_AC vm->state.registers.ac
#define MORO8_X vm->state.registers.x
#define MORO8_Y vm->state.registers.y
#define MORO8_SR vm->state.registers.sr
#define MORO8_SP vm->state.registers.sp
#define MORO8_C vm->state.registers.sr.c
#define MORO8_Z vm->state.registers.sr.z
#define MORO8_V vm->state.registers.sr.v
#define MORO8_N vm->state.registers.sr.n

typedef struct moro8_state moro8_state;
...
{
    memset(&vm->state, 0, sizeof(moro8_state));
}

const void* moro8_as_buffer(const moro8_vm* vm, size_t* size)
{
    *size = sizeof(moro8_state);
    return (const void*)&vm->state;
}

void moro8_from_buffer(moro8_vm* vm, const void* buf, size_t size)
{
    size_t state_size = sizeof(moro8_state);
    memset(&vm->state, 0, state_size);
    memcpy(&vm->state, buf, state_size < size ? state_size : size);
}

void moro8_backup(moro8_vm* snapshot, const moro8_vm* vm)
{
    memcpy(&snapshot->state, &vm->state, sizeof(moro8_state));
}

void moro8_restore(moro8_vm* vm, const moro8_vm* snapshot)
{
    memcpy(&vm->state, &snapshot->state, sizeof(moro8_state));
}
...
int moro8_equal(const moro8_vm* left, const moro8_vm* right)
{
    ...
    return memcmp(&left->state, &right->state, sizeof(moro8_state)) == 0;
}

void moro8_load(moro8_vm* vm, const moro8_uword* prog, moro8_udword size)
{
    moro8_reset(vm);
    memcpy(&vm->state.memory[MORO8_PROGMEM_OFFSET], prog, size);
}
...
{
    moro8_uword instruction = vm->state.memory[vm->state.registers.pc];
    ...
    moro8_uword operand = vm->state.memory[++vm->state.registers.pc];

/** Get a double word operand */
#define MORO8_DWORD_OPERAND (operand + (vm->state.memory[++vm->state.registers.pc] << 8))
/** Get a double word at a memory address */
#define MORO8_MEMORY_DWORD(addr) ((moro8_udword)vm->state.memory[addr] + ((addr < MORO8_MEMORY_SIZE - 1) ? (((moro8_udword)vm->state.memory[addr + 1]) << 8) : 0))
#define MORO8_MEMORY_SET_DWORD(addr, value) vm->state.memory[addr] = MORO8_LOW(value); if (addr < MORO8_MEMORY_SIZE - 1) { vm->state.memory[addr + 1] = MORO8_HIGH(value); }
#define MORO8_DEC_PC --vm->state.registers.pc
#define MORO8_TWO_COMPLEMENT(value) (~result + 1)
#define MORO8_MEM(addr) vm->state.memory[addr]
...
{ \
    ...
    case MORO8_OP_JMP_ABS:
        vm->state.registers.pc = MORO8_DWORD_OPERAND - 1;
        break;
    case MORO8_OP_JMP_IND:
    {
        moro8_udword addr = MORO8_DWORD_OPERAND;
        vm->state.registers.pc = MORO8_MEMORY_DWORD(addr) - 1;
        break;
    }
    ...
    case MORO8_OP_ORA_IND_Y:
        ...
        break;
    case MORO8_OP_PHA:
        vm->state.memory[vm->state.registers.sp] = MORO8_AC;
        vm->state.registers.sp++;
        break;
    case MORO8_OP_PLA:
        vm->state.registers.sp--;
        MORO8_SET_AC(vm->state.memory[vm->state.registers.sp]);
        break;
    ...
    vm->state.registers.pc++;
    return MORO8_TRUE;
}

void moro8_run(moro8_vm* vm)
{
    vm->state.registers.pc = MORO8_PROGMEM_OFFSET;
    moro8_resume(vm);
}

moro8_udword moro8_get_pc(const moro8_vm* vm)
{
    return vm->state.registers.pc;
}

void moro8_set_pc(moro8_vm* vm, moro8_udword value)
{
    vm->state.registers.pc = value;
}
...
moro8_udword moro8_get_memory(const moro8_vm* vm, moro8_uword* buffer, moro8_udword offset, moro8_udword size)
{
    ...
    memcpy(buffer, vm->state.memory + offset, size);
    return size;
}
...
{
    const moro8_uword* mem = &vm->state.memory[0];
...
char* moro8_print(const moro8_vm* vm)
{
    ...
    char* ptr = buf;
    ptr = moro8_print_register_dword(ptr, "PC:", vm->state.registers.pc);
    ptr = moro8_print_register_word(ptr, "AC:", vm->state.registers.ac);
    ptr = moro8_print_register_word(ptr, "X: ", vm->state.registers.x);
    ptr = moro8_print_register_word(ptr, "Y: ", vm->state.registers.y);
    ptr = moro8_print_register_word(ptr, "SP:", vm->state.registers.sp);
    ptr = moro8_print_register_word(ptr, "N: ", vm->state.registers.sr.n);
    ptr = moro8_print_register_word(ptr, "V: ", vm->state.registers.sr.v);
    ptr = moro8_print_register_word(ptr, "Z: ", vm->state.registers.sr.z);
    ptr = moro8_print_register_word(ptr, "C: ", vm->state.registers.sr.c);
    *ptr++ = '\n';
        ...
        if (state == MORO8_STATE_IDLE)
        {
            ...
            for (size_t j = 0; i < size; ++j, ++i)
            {
                if (buf[j] == ':')
                {
                    ...
                    {
                        // Found an address
                        value_buffer = &vm->state.memory[address];
                    }
                    else if (buf[0] == 'P' && buf[1] == 'C')
                    {
                        vm->state.registers.pc = 0;
                        value_buffer = (moro8_uword*)&vm->state.registers.pc;
                    }
                    else if (buf[0] == 'A' && buf[1] == 'C')
                    {
                        value_buffer = &vm->state.registers.ac;
                    }
                    else if (buf[0] == 'X')
                    {
                        value_buffer = &vm->state.registers.x;
                    }
                    else if (buf[0] == 'Y')
                    {
                        value_buffer = &vm->state.registers.y;
                    }
                    else if (buf[0] == 'S' && buf[1] == 'P')
                    {
                        value_buffer = &vm->state.registers.sp;
                    }
                    else if (buf[0] == 'N')
                    {
                        value_buffer = &vm->state.registers.sr.n;
                    }
                    else if (buf[0] == 'V')
                    {
                        value_buffer = &vm->state.registers.sr.v;
                    }
                    else if (buf[0] == 'C')
                    {
                        value_buffer = &vm->state.registers.sr.c;
                    }
                    else if (buf[0] == 'Z')
                    {
                        value_buffer = &vm->state.registers.sr.z;
                    }
                    else
                    {
                        // Unknown label
