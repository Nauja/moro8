...
static void test_bus(void** state) {
    ...
    memory.bus.set(&memory, 0xFF, 0);
    assert_int_equal(memory.bus.get(&memory, 0xFF), 0);
    memory.bus.set(&memory, 0xFF, 0xFF);
    assert_int_equal(memory.bus.get(&memory, 0xFF), 0xFF);
}
